...
class mth_prediction(tf.keras.layers.Layer):

  def __init__(self, parameter_list, encoder, decoder, koopman_aux_net, koopman_jordan):
        super(mth_prediction, self).__init__()
        print('Making mth_pred')
        self.encoder = encoder
        self.koopman_aux_net = koopman_aux_net
        self.koopman_jordan = koopman_jordan
  ...
  def call(self, inputs):
        ...
        return next_state_space_mth_list
...
class Koopman_RNN(tf.keras.Model):
    ...
    def __init__(self, parameter_list, **kwargs):
        ...
        self.mth_prediction = mth_prediction(parameter_list, self.encoder, self.decoder, self.koopman_aux_net, self.koopman_jordan)
        self.mth_step = parameter_list['mth_step']
    ...
    def call(self, inputs, cal_mth_loss = False):
        ...
        if cal_mth_loss:
            next_state_space_mth_list = self.mth_prediction(inputs)
