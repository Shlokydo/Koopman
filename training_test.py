...
def train(parameter_list, model, checkpoint, manager, summary_writer, optimizer):
    ...
    with summary_writer.as_default():
        ...
        for epoch in range(epochs):
            ...
            epoch = parameter_list['global_epoch'] + epoch + 1
            ...
            for step, (t_current, t_next_actual) in enumerate(dataset):
                ...
                with tf.GradientTape() as tape:
                    ...
                    if cal_mth_loss_flag:
                        loss_mth_prediction = loss_func(t_mth_predictions, t_next_actual[:, parameter_list['mth_step']:,:]) / tf.reduce_mean(tf.square(t_next_actual[:, parameter_list['mth_step']:,:]))
            ...
            for step_val, (t_current_val, t_next_actual_val) in enumerate(val_dataset):
                ...
                if cal_mth_loss_flag_val:
                    val_loss_mth_prediction = loss_func(t_next_actual_val[:, parameter_list['mth_step']:,:], t_mth_predictions_val) / tf.sqrt(tf.reduce_mean(tf.square(t_next_actual_val[:, parameter_list['mth_step']:,:])))
